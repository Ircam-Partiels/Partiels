...
- Track Reference: This property defines which track is used as a reference for the navigation and the edition of the group. The vertical zoom of the group is automatically based on the frontmost track (top-most in the layout) that has vertical zoom capability. As tracks can have different vertical zoom ranges or none (when tracks contain markers), the group automatically selects the first suitable track for displaying the vertical ruler and scrollbar. By default, the group uses the frontmost track as the reference, but you can select another track if it does not meet your needs.
