...
void AnlVampPlugin::Stats::setPreComputingFeatures(FeatureSet const& fs)
{
    ...
    static auto const epsilonRt = Vamp::RealTime::fromSeconds(std::numeric_limits<double>::epsilon());

    auto const perform = [&, this](Vamp::RealTime const& start, Vamp::RealTime const& end)
    {
        auto const cmp = [](auto const& f, auto const& t)
        {
            return f.timestamp < t;
        };
        auto const firstIt = std::lower_bound(lastPoint, points.cend(), start, cmp);
        auto const nextIt = std::lower_bound(firstIt, points.cend(), end, cmp);
        auto const secondIt = nextIt != points.cend() && nextIt != firstIt ? std::prev(nextIt) : nextIt;
        ...
        auto const duration = end - start - epsilonRt;
        if(!sorted.empty())
        {
