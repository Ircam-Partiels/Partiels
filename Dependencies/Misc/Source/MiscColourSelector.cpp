...
void ColourSelector::HueSelector::paint(juce::Graphics& g)
{
    ...
    for(auto i = 0; i <= 5; i++)
    {
        auto const position = static_cast<float>(i) / 5.0f;
        cg.addColour(position, juce::Colour(position, 1.0f, 1.0f, 1.0f));
    }
...
void ColourSelector::HueSelector::mouseDrag(juce::MouseEvent const& event)
{
    ...
    hsba[0] = std::clamp(static_cast<float>(event.x) / static_cast<float>(getWidth()), 0.0f, 1.0f);
...
{
    auto hsba = mOwner.getHSBA();
    hsba[1] = std::clamp(static_cast<float>(event.x) / static_cast<float>(getWidth()), 0.0f, 1.0f);
    hsba[2] = std::clamp(1.0f - static_cast<float>(event.y) / static_cast<float>(getHeight()), 0.0f, 1.0f);
    mOwner.setHSBA(hsba, juce::NotificationType::sendNotificationSync);
}
...
, mIndex(index)
{
    ...
    {
        updateValue(static_cast<juce::uint8>(std::clamp(mValue.getText().getIntValue(), 0, 255)));
    };
    ...
    {
        updateValue(static_cast<juce::uint8>(std::clamp(mSlider.getValue(), 0.0, 255.0)));
    };
...
: juce::Button("ColourButton")
{
    ...
    mComponentListener.onComponentBeingDeleted = [this](juce::Component& component)
    {
        ...
        MiscWeakAssert(mIsColourSelectorVisible == true);
...
void ColourButton::mouseDrag(juce::MouseEvent const& event)
{
    ...
    if(dragContainer != nullptr && !dragContainer->isDragAndDropActive())
    {
        ...
        dragContainer->startDragging({}, this, juce::ScaledImage{}, true, &p, &event.source);
    }
}
...
{
    MiscWeakAssert(mIsColourSelectorVisible == false);
...
void ColourButton::itemDragEnter(juce::DragAndDropTarget::SourceDetails const& dragSourceDetails)
{
    ...
    MiscWeakAssert(mDraggedColour.getComponent() != nullptr);
