...
std::tuple<juce::Result, juce::String> Document::Director::addTrack(juce::String const groupIdentifer, size_t position, NotificationType const notification)
{
    MiscStrongAssert(Tools::hasGroupAcsr(mAccessor, groupIdentifer));
    if(!Tools::hasGroupAcsr(mAccessor, groupIdentifer))
    {
        return std::make_tuple(juce::Result::fail(juce::translate("The specified group \"GROUPID\" does not exist.").replace("GROUPID", groupIdentifer)), juce::String());
    }
...
std::tuple<juce::Result, juce::String> Document::Director::copyTrack(juce::String const groupIdentifier, size_t index, juce::String const trackIdentifier, NotificationType const notification)
{
    ...
    auto [addResult, newTrackIdentifier] = addTrack(groupIdentifier, index, notification);
    if(addResult.failed())
    {
...
std::tuple<juce::Result, juce::String> Document::Director::copyGroup(juce::String const groupIdentifier, size_t index, NotificationType const notification)
{
    ...
    for(size_t i = 0; i < trackIdentifiers.size(); ++i)
    {
        ...
        {
            std::make_tuple(newTrackResult, juce::String());
        }
    }
