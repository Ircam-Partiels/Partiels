...
void Document::Exporter::Panel::updateItemPopup()
{
        ...
        for(auto const& groupId : documentLayout)
        {
            ...
            else
            {
                ...
                for(auto const& trackId : groupLayout)
                {
                    ...
                    if(mOptions.isCompatible(frameType) && !mSelectedIdentifiers.count(trackId))
                    {
                        allSelected = false;
                        break;
                    }
            ...
            for(auto const& groupId : documentLayout)
            {
                ...
                else
                {
                    ...
                    for(auto const& trackId : groupLayout)
                    {
                        auto const& trackAcsr = Document::Tools::getTrackAcsr(mAccessor, trackId);
                        auto const frameType = Track::Tools::getFrameType(trackAcsr);
                        if(mOptions.isCompatible(frameType))
                        {
                            mSelectedIdentifiers.insert(trackId);
                        }
                    }
    ...
    for(auto const& groupId : documentLayout)
    {
        ...
        if(mOptions.useImageFormat())
        {
            ...
            menu.addItem(juce::translate("GROUPNAME (Group)").replace("GROUPNAME", groupName), true, groupSelected, [this, groupId, toggleSelection]()
                         {
        ...
        for(auto const& trackId : groupLayout)
        {
            ...
            auto const frameType = Track::Tools::getFrameType(trackAcsr);
            bool const isCompatible = mOptions.isCompatible(frameType);
            bool const trackSelected = mSelectedIdentifiers.count(trackId) > 0;

            menu.addItem(juce::translate("GROUPNAME: TRACKNAME (Track)")
...
juce::Result Document::Exporter::toFile(Accessor& accessor, juce::File const file, juce::Range<double> const& timeRange, std::set<size_t> const& channels, juce::String const filePrefix, std::set<juce::String> const& identifiers, Options const& options, std::atomic<bool> const& shouldAbort)
{
    if(file == juce::File())
    {
    ...
    {
        std::vector<std::reference_wrapper<Track::Accessor>> result;
        for(auto const& identifier : identifiers)
        {
            if(Tools::hasGroupAcsr(accessor, identifier))
            {
                auto groupTracks = Group::Tools::getTrackAcsrs(Tools::getGroupAcsr(accessor, identifier));
                result.insert(result.end(), groupTracks.begin(), groupTracks.end());
            }
            else if(Tools::hasTrackAcsr(accessor, identifier))
            {
                result.push_back(Tools::getTrackAcsr(accessor, identifier));
            }
        }
        return result;
    };
    ...
    if(options.useImageFormat())
    {
        ...
        // Export all items in the identifiers set
        for(auto const& identifier : identifiers)
        {
            if(Tools::hasGroupAcsr(accessor, identifier))
            {
    ...
    auto const exportTrack = [&](juce::String const& trackIdentifier, juce::File const& trackFile)
    {
        ...
        auto const& trackAcsr = Tools::getTrackAcsr(accessor, trackIdentifier);
        auto const isInIdentifiers = identifiers.count(trackIdentifier) > 0;
        if(!isInIdentifiers && options.ignoreGridResults && Track::Tools::getFrameType(trackAcsr) == Track::FrameType::vector)
        {
            return juce::Result::ok();
        }
        auto const isLabelOnly = options.format == Options::Format::cue || options.format == Options::Format::reaper;
        if(!isInIdentifiers && isLabelOnly && Track::Tools::getFrameType(trackAcsr) != Track::FrameType::label)
        {
            return juce::Result::ok();
        }
        auto const fileUsed = trackFile.isDirectory() ? trackFile.getNonexistentChildFile(filePrefix + trackAcsr.getAttr<Track::AttrType::name>(), "." + options.getFormatExtension()) : trackFile.getSiblingFile(filePrefix + trackFile.getFileName());
