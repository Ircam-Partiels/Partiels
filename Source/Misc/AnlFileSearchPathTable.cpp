...
void FileSearchPathTable::lookAndFeelChanged()
{
    auto* laf = dynamic_cast<IconManager::LookAndFeelMethods*>(&getLookAndFeel());
    anlWeakAssert(laf != nullptr);
    if(laf != nullptr)
...
void FileSearchPathTable::parentHierarchyChanged()
{
    lookAndFeelChanged();
}
...
bool FileSearchPathTable::deleteSelection()
{
    ...
    {
        anlWeakAssert(*it < fileSearchPath.size());
...
bool FileSearchPathTable::copySelection()
{
    ...
    for(auto it = mSelection.crbegin(); it != mSelection.crend(); ++it)
    {
        anlWeakAssert(*it < mFileSearchPath.size());
