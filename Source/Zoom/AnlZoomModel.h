    ...
    public:
        ...
        void setAttr(value_v const& value, NotificationType notification)
        {
            if constexpr(type == AttrType::anchor)
            {
                ...
                Model::Accessor<Accessor, AttrContainer, AcsrContainer>::setAttr<AttrType::anchor, value_v>(std::make_tuple(std::get<0>(value), position), notification);
            }
            else if constexpr(type == AttrType::visibleRange)
            {
                ...
                Model::Accessor<Accessor, AttrContainer, AcsrContainer>::setAttr<AttrType::visibleRange, Zoom::Range>(sanitize(value, getAttr<AttrType::globalRange>(), getAttr<AttrType::minimumLength>()), notification);
            }
            else
            {
