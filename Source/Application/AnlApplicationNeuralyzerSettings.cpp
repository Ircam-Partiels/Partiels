...
: mAccessor(accessor)
, mBackend(juce::translate("Backend"), juce::translate("The backend used to run Neuralyzer"), "", {juce::translate("None"), juce::translate("Local"), juce::translate("Remote")}, [&](size_t index)
           {
...
, mRemoteUrl(juce::translate("Server URL"), juce::translate("The URL of the remote server (scheme and host, e.g. http://localhost:1234)"), [&](juce::String text)
             {
                 ...
                 modelInfo.serverUrl = juce::URL(text);
...
, mNeuralyzerDirectory(juce::translate("Neuralyzer Directory"), juce::translate("Reveal the directory where Neuralyzer data is stored"), []()
                       {
                           ...
                           resolveDirectory(root).revealToUser();
                       })
, mRagEngineDownload(juce::translate("Download RAG models"), juce::translate("Download the RAG embedding and reranker models automatically"), []()
                     {
                         Rag::downloadModelsIfNecessary();
                     })
, mMcpForClaudeApp(juce::translate("MCP for the Claude Desktop Application"), juce::translate("MCP for the Claude Desktop Application"), [&](bool state)
                   {
                       mAccessor.setAttr<AttrType::mcpForClaudeApp>(state, NotificationType::synchronous);
                   })
, mMcpForCopilotApp(juce::translate("MCP for the GitHub Copilot"), juce::translate("MCP for the GitHub Copilot"), [&](bool state)
                    {
                        mAccessor.setAttr<AttrType::mcpForCopilotApp>(state, NotificationType::synchronous);
                    })
{
    ...
    addAndMakeVisible(mNeuralyzerDirectory);
    addAndMakeVisible(mRagEngineDownload);
    addAndMakeVisible(mMcpSeparator);
    mMcpSeparator.setSize(1, 1);
    addAndMakeVisible(mMcpForClaudeApp);
    addAndMakeVisible(mMcpForCopilotApp);
    ...
    mListener.onAttrChanged = [this](Accessor const& acsr, AttrType attr)
    {
        switch(attr)
        {
            ...
            {
                mMcpForClaudeApp.entry.setToggleState(acsr.getAttr<AttrType::mcpForClaudeApp>(), juce::NotificationType::dontSendNotification);
                break;
            }
            case AttrType::mcpForCopilotApp:
            {
                mMcpForCopilotApp.entry.setToggleState(acsr.getAttr<AttrType::mcpForCopilotApp>(), juce::NotificationType::dontSendNotification);
                break;
            }
        }
        ...
        mRemoteUrl.entry.setText(modelInfo.serverUrl.toString(true), juce::NotificationType::dontSendNotification);
...
void Application::Neuralyzer::SettingsContent::resized()
{
    ...
    setBounds(mNeuralyzerDirectory);
    setBounds(mRagEngineDownload);
    for(auto const& view : mDownloadViews)
    {
        setBounds(*view.get());
    }
    setBounds(mMcpSeparator);
    setBounds(mMcpForClaudeApp);
    setBounds(mMcpForCopilotApp);
...
void Application::Neuralyzer::SettingsContent::handleCommandMessage([[maybe_unused]] int commandId)
{
    ...
    mRagEngineDownload.setVisible(!hasEmbeddingModel && !hasRerankerModel);
    resized();
}
