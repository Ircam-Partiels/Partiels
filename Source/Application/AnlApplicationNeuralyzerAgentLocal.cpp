    ...
    for(auto iteration = 0_z; iteration < maxIterations && mShouldQuit.load() == false; ++iteration)
    {
        ...
        else
        {
            ...
            auto const [parsingError, toolCalls] = [&, inputMsg = assistantMsg]()
            {
                ...
                {
                    MiscDebug("Application::Neuralyzer::AgentLocal", "Exception during tool call parsing: " + juce::String(e.what()));
...
juce::Result Application::Neuralyzer::AgentLocal::loadSession(juce::File const& sessionFile)
{
    ...
    auto const loadMessages = [&]()
    {
        ...
        MiscDebug("Application::Neuralyzer::AgentLocal", juce::String("Restored ") + juce::String(tokens.size()) + juce::String(" tokens from message history: ") + contextFile.getFullPathName());
        return juce::Result::ok();
    };
    ...
    if(nloaded == 0)
    {
        MiscDebug("Application::Neuralyzer::AgentLocal", "Failed to load state from file (no tokens loaded): " + contextFile.getFullPathName());
    }
    updateContextMemoryUsage();
