...
struct Application::Neuralyzer::ChatHistory::ParseState
{
    ...
    juce::Colour const textColour = editor.findColour(juce::TextEditor::ColourIds::textColourId);
    juce::Colour const backgroundColour = editor.findColour(juce::TextEditor::ColourIds::backgroundColourId);
    ...
    void insertText(juce::String const& text)
    {
        ...
        {
            auto const colour = ((isLink || isInTable) ? juce::Colours::blue : textColour).interpolatedWith(backgroundColour, isQuote ? 0.65f : 0.0f);
            editor.setColour(juce::TextEditor::ColourIds::textColourId, colour);
            editor.setFont(getCurrentFont());
            editor.insertTextAtCaret(text);
            atLineStart = text.endsWithChar('\n');
            editor.setColour(juce::TextEditor::ColourIds::textColourId, textColour);
        }
    }
    ...
    static int onEnterBlock(MD_BLOCKTYPE type, void* detail, void* userdata)
    {
        ...
        switch(type)
        {
            ...
            case MD_BLOCK_BLANK:
            {
                if(auto const* blankDetail = static_cast<MD_BLOCK_BLANK_DETAIL const*>(detail))
                {
                    auto count = blankDetail->line_count;
                    while(count-- > 0)
                    {
                        state.insertText("\n");
                    }
        ...
        switch(type)
        {
            ...
            case MD_SPAN_A:
            {
                if(auto* link = static_cast<MD_SPAN_A_DETAIL const*>(detail))
                {
                    state.isLink = true;
                    auto const start = state.editor.getCaretPosition();
                    auto text = juce::String::fromUTF8(link->href.text, static_cast<int>(link->href.size));
                    auto const fileRef = juce::StringRef("file://");
                    auto const fileIndex = text.indexOf(fileRef);
                    if(fileIndex >= 0)
                    {
                        text = text.replaceSection(0, fileIndex + fileRef.length(), fileRef);
                        text = text.upToLastOccurrenceOf(">", false, true);
                    }
                    state.links.push_back(std::make_pair(juce::Range<int>{start, start}, juce::URL(text)));
                }
                break;
            }
...
void Application::Neuralyzer::ChatHistory::mouseDown(juce::MouseEvent const& event)
{
    ...
    if(linkIt != mLinks.cend())
    {
        ...
        {
            auto const file = linkIt->second.getLocalFile();
            ...
            else
            {
                auto const options = juce::MessageBoxOptions()
                                         ...
                                         .withTitle(juce::translate("The file doesn't exist!"))
                                         .withMessage(juce::translate("The file FLPATH doesn't exist.").replace("FLPATH", file.getFullPathName()))
...
void Application::Neuralyzer::ChatHistory::setHistory(History const& history)
{
    ...
    auto const addText = [&](auto const& role, auto const& request, auto const& colour)
    {
        ...
        parser.flags = MD_FLAG_PERMISSIVEAUTOLINKS | MD_FLAG_TABLES | MD_FLAG_STRIKETHROUGH | MD_FLAG_TASKLISTS | MD_FLAG_HARD_SOFT_BREAKS | MD_FLAG_PRESERVEBLANKLINES;
