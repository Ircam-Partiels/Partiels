...
std::vector<Application::Neuralyzer::Rag::Resource> Application::Neuralyzer::Rag::createResources(Ast::Document const& document)
{
    ...
    for(auto const& section : document.sections)
    {
        ...
        static auto constexpr maxContextLength = 500;
        auto lines = juce::StringArray::fromLines(section.content.replace("\r", ""));
        lines.trim();
        lines.removeEmptyStrings();
        for(auto const& line : lines)
        {
            if(resource.content.length() + line.length() > maxContextLength)
            {
                addResource();
            }
            if(line.length() > maxContextLength)
            {
                addResource();
                auto tokens = juce::StringArray::fromTokens(" ", true);
                for(auto const& token : tokens)
                {
                    if(resource.content.length() + token.length() > maxContextLength)
                    {
                        addResource();
                    }
                    if(token.length() > maxContextLength)
                    {
                        addResource();
    ...
    if(listResults.contains("resources") && listResults.at("resources").is_array())
    {
        for(auto const& resource : listResults.at("resources"))
        {
            ...
            if(contentResult.contains("contents") && contentResult.at("contents").is_array())
            {
                ...
                {
                    auto const text = content.value("text", juce::String{});
                    auto const ast = Neuralyzer::Ast::parseMarkdown(text, name, false);
                    auto const resources = Neuralyzer::Rag::createResources(ast);
...
float Application::Neuralyzer::Rag::Engine::computeRerankerScore(juce::String const& query, juce::String const& resourceContent)
{
    ...
    auto const* logits = llama_get_logits_ith(context, tokenizedCount - 1);
    if(logits == nullptr)
    {
        return 0.0f;
    }
...
std::vector<Application::Neuralyzer::Rag::Resource> Application::Neuralyzer::Rag::Engine::computeResources(juce::String const& userPrompt, float minScore, float maxDistance)
{
    ...
    auto const topK = std::min(12_z, mIndexedEntries.size());
    ...
    static auto constexpr maxRerankerResources = 2;
    ...
    if(mRerankerModel != nullptr && mRerankerContext != nullptr)
    {
        ...
        resources.clear();
        for(auto i = 0_z; i < rankedByReranker.size() && resources.size() < maxRerankerResources; ++i)
        {
