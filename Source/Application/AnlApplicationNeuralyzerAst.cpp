...
namespace
{
    ...
    static int astEnterBlock(MD_BLOCKTYPE type, void* detail, void* userdata)
    {
        ...
        switch(type)
        {
            ...
            case MD_BLOCK_UL:
            {
                state.listIndices.push_back(0_z);
                break;
            }
            case MD_BLOCK_OL:
            {
                state.listIndices.push_back(1_z);
                break;
            }
            case MD_BLOCK_LI:
            {
                auto const numList = static_cast<int>(state.listIndices.size());
                section.content << juce::String::repeatedString("  ", numList);
                if(state.listIndices.back() == 0_z)
                {
                    section.content << "- ";
                }
                else
                {
                    section.content << state.listIndices.back() << ". ";
                    ++state.listIndices.back();
                }
                break;
            }
            case MD_BLOCK_P:
    ...
    static int astEnterSpan(MD_SPANTYPE type, void* detail, void* userdata)
    {
        ...
        switch(type)
        {
            case MD_SPAN_A:
            {
                ...
                {
                    content << "[URL: " + juce::String::fromUTF8(link->href.text, static_cast<int>(link->href.size)) + "]";
                    content << "[Title: " + juce::String::fromUTF8(link->title.text, static_cast<int>(link->title.size)) + "] ";
                }
                break;
            }
            case MD_SPAN_IMG:
