...
void Application::CommandTarget::getAllCommands(juce::Array<juce::CommandID>& commands)
{
    ...
    commands.addArray(
    {
        ...
        , CommandIDs::viewShowNeuralyzerPanel
...
void Application::CommandTarget::getCommandInfo(juce::CommandID const commandID, juce::ApplicationCommandInfo& result)
{
    ...
    switch(commandID)
    {
        ...
        case CommandIDs::viewShowNeuralyzerPanel:
        {
            auto* window = Instance::get().getWindow();
            auto const isVisible = window != nullptr && window->getInterface().isNeuralyzerPanelVisible();
            result.setInfo(isVisible ? juce::translate("Hide Neuralyzer Panel") : juce::translate("Show Neuralyzer Panel"),
                           juce::translate("Shows or hides the Neuralyzer panel"), "View", 0);
            ...
            juce::AlertWindow::showAsync(options, [=, this](int windowResult)
                                         {
                                             ...
                                             auto const& lDocumentAcsr = Instance::get().getDocumentAccessor();
                                             for(auto const& trackId : trackIds)
                                             {
                                                 if(Document::Tools::hasTrackAcsr(lDocumentAcsr, trackId))
                                                 {
                                                     auto const& trackAcsr = Document::Tools::getTrackAcsr(lDocumentAcsr, trackId);
