...
std::tuple<juce::Result, std::string> Application::Neuralyzer::Agent::sendUserQuery(juce::String const& prompt, bool allowTools)
{
    ...
    for(int iteration = 0; iteration < maxIterations && mShouldQuit.load() == false; ++iteration)
    {
        ...
        else
        {
            ...
            {
                return std::make_tuple(juce::Result::ok(), std::get<1>(result));
            }
            else if(tpResult.first.wasOk())
            {
                ...
                else
                {
                    ...
                    {
                        return std::make_tuple(juce::Result::fail(juce::translate("Tool call failed: ") + tcResult.first.getErrorMessage()), finalResponse);
                    }
                }
            }
            else
            {
                ...
                {
                    return std::make_tuple(juce::Result::fail(juce::translate("Tool parsing failed: ") + tpResult.first.getErrorMessage()), finalResponse);
                }
            }
        }
    }
