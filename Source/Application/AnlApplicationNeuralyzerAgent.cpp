...
std::string Application::Neuralyzer::Agent::callTools(Mcp::Dispatcher& mcpDispatcher, Mcp::Dispatcher::DynamicToolMethods const& mcpMethods, std::vector<common_chat_tool_call> const& toolCalls, size_t const maxErrors, size_t& numErrors)
{
    auto const [callSucceeded, toolResponses] = mcpDispatcher.callTools(mcpMethods, toolCalls);
