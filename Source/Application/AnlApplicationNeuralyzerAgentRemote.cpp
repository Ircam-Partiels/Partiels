...
Application::Neuralyzer::AgentRemote::AgentRemote(Mcp::Dispatcher& mcpDispatcher, Guard& guard, Rag::Engine& rag)
...
juce::Result Application::Neuralyzer::AgentRemote::sendQuery(juce::String const& prompt)
{
    ...
    {
        std::unique_lock<std::mutex> sessionLock(mSessionMutex);
        mHistory.push_back({MessageType::warning, prompGuardResult.getErrorMessage()});
        notifyStateChanged();
        return juce::Result::ok();
    }
