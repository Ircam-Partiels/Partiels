...
std::tuple<juce::Result, std::vector<common_chat_tool_call>> Application::Neuralyzer::AgentRemote::performMessage(MessageType const& role, juce::String const& prompt, bool allowTools)
{
    ...
    if(allowTools)
    {
        request["tools"] = common_chat_tools_to_json_oaicompat(mMcpDispatcher.getToolList(mMcpMethods));
    }
...
juce::Result Application::Neuralyzer::AgentRemote::sendQuery(juce::String const& prompt, juce::StringArray const& files)
{
    {
        std::unique_lock<std::mutex> sessionLock(mSessionMutex);
        mTempResponse.clear();
...
{
    auto const instructions = mMcpDispatcher.getInstructions();
    {
        std::unique_lock<std::mutex> sessionLock(mSessionMutex);
        mHistory.clear();
        mTempResponse.clear();
        mHistory.push_back({MessageType::system, instructions});
    }
    return juce::Result::ok();
}
...
juce::Result Application::Neuralyzer::AgentRemote::saveSession(juce::File const& sessionFile)
{
    ...
    if(history.size() <= kNumInitMessages)
    {
    ...
    try
    {
        ...
        {
            nlohmann::json msg;
            msg["role"] = magic_enum::enum_name(type);
...
juce::Result Application::Neuralyzer::AgentRemote::loadSession(juce::File const& sessionFile)
{
    ...
    try
    {
        ...
        {
            auto type = magic_enum::enum_cast<MessageType>(msgJson.value("role", std::string{})).value_or(MessageType::user);
            auto content = type != MessageType::system ? msgJson.value("content", juce::String{}) : juce::String(mMcpDispatcher.getInstructions());
            history.push_back({std::move(type), std::move(content)});
        }
