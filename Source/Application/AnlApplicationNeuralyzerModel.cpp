...
juce::File Application::Neuralyzer::getTempDirectory()
{
    return juce::File::getSpecialLocation(juce::File::SpecialLocationType::tempDirectory).getChildFile("Neuralyzer");
}

juce::File Application::Neuralyzer::getTempHistoryFile()
{
    return getTempDirectory().getChildFile("CurrentHistory.md");
}

juce::File Application::Neuralyzer::getTempExportDirectory()
{
    return getTempDirectory().getChildFile("Export");
}

juce::File Application::Neuralyzer::getTempSessionFile(juce::String const& name)
{
    auto const hash = juce::String::toHexString(static_cast<juce::int64>(name.hashCode64()));
    return getTempDirectory().getChildFile("Sessions").getChildFile(hash + ".messages.json");
}
