...
void Application::FileManager::openAudioFiles(std::vector<juce::File> const& files)
{
    ...
    auto const audioFileWildcards = Instance::getWildCardForAudioFormats();
    std::vector<AudioFileLayout> readerLayout;
    for(auto const& file : files)
    {
        auto const fileExtension = file.getFileExtension().toLowerCase();
        MiscWeakAssert(file.existsAsFile() && file.hasReadAccess() && audioFileWildcards.contains(fileExtension));
        if(file.existsAsFile() && file.hasReadAccess() && audioFileWildcards.contains(fileExtension))
        {
    ...
    {
        juce::WeakReference<Instance> weakReference(&Instance::get());
        fileBased.saveIfNeededAndUserAgreesAsync([=, rl = readerLayout](juce::FileBasedDocument::SaveResult saveResult)
                                                 {
                                                     if(weakReference.get() == nullptr)
                                                     {
                                                         return;
...
{
    Instance::get().getDocumentFileBased().saveAsync(true, true, nullptr);
}
