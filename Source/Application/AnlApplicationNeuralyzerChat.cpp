...
void Application::Neuralyzer::Chat::mouseDown(juce::MouseEvent const& event)
{
    ...
    {
        if(!mAgent.getModelInfo().isValid() && !mAgent.getAccessor().getAttr<AttrType::modelInfo>().isValid())
        {
            askToConfigureNeuralyzer();
        }
    }
...
void Application::Neuralyzer::Chat::handleAsyncUpdate()
{
    ...
    if(currentAction == BackgroundAgent::Action::none)
    {
        ...
        else
        {
            ...
            mQueryEditor.setTextToShowWhenEmpty(juce::translate("Model not initialized"), findColour(juce::TextEditor::ColourIds::textColourId).withAlpha(0.5f));
        }
...
void Application::Neuralyzer::Chat::sendUserQuery()
{
    ...
    mAgent.sendQuery(query, mcpToolsContext);
...
void Application::Neuralyzer::Chat::updateHistory()
{
    ...
    auto const errorColour = juce::Colours::orange;
