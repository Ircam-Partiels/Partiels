...
namespace
{
    static juce::Result downloadModel(juce::File outputDirectory, juce::URL sourceUrl, std::function<bool(int, int)> callback)
    {
        if(callback != nullptr)
        {
            callback(0, 100);
        ...
        {
            return juce::Result::fail(juce::translate("Invalid URL \"URLPATH\"").replace("URLPATH", sourceUrl.toString(true)));
        }

        // Download model
        static auto constexpr connectionTimeoutMs = 1000;
        auto const target = outputDirectory.getChildFile(sourceUrl.getFileName());
        ...
        if(!target.existsAsFile())
        {
            ...
            auto const result = Downloader::download(sourceUrl, target, options);
            if(result.failed())
            {
                target.deleteFile();
                return result;
            ...
            {
                target.deleteFile();
                return juce::Result::fail(juce::translate("Failed to download from URL \"URLPATH\": HTTP STATUS.").replace("URLPATH", sourceUrl.toString(true)).replace("STATUS", juce::String(statusCode)));
            }
            if(!shouldContinue.load())
            {
                target.deleteFile();
                return juce::Result::fail(juce::translate("Download from URL \"URLPATH\" cancelled by user.").replace("URLPATH", sourceUrl.toString(true)));
            }
        }
        if(callback != nullptr)
        {
...
Application::Neuralyzer::Downloader::Process::Process(juce::File outputDirectory, juce::URL sourceUrl, std::function<void(Process const&)> callback)
: mCallback(std::move(callback))
{
    auto const createResult = outputDirectory.createDirectory();
    if(createResult.failed())
    {
        mResult = juce::Result::fail(juce::translate("The output directory 'DIRPATH' cannot be created: ERROR").replace("DIRPATH", outputDirectory.getFullPathName()).replace("ERROR", createResult.getErrorMessage()));
        triggerAsyncUpdate();
        return;
    }

    mOutputFile = outputDirectory.getChildFile(sourceUrl.getFileName());
    mIsRunning.store(true);
    mAsyncProcess = std::async([this, outputDirectory, sourceUrl]()
                               {
                                   juce::Thread::setCurrentThreadName("Application::Neuralyzer::Downloader");
                                   auto result = downloadModel(outputDirectory, sourceUrl, [this](int avd, int total)
                                                               {
...
bool Application::Neuralyzer::Downloader::Manager::start(juce::File output, juce::URL url, std::function<void(Process const&)> callback)
{
    auto const OutputFile = output.getChildFile(url.getFileName());
    auto const isAlreadyDownloading = std::any_of(mProcesses.cbegin(), mProcesses.cend(), [&](auto const& process)
                                                  {
                                                      return process->isRunning() && process->getOutputFile() == OutputFile;
                                                  });
    ...
    auto process = std::make_unique<Process>(std::move(output), std::move(url), std::move(callback));
