    ...
    static nlohmann::json performToolsCall(nlohmann::json const& request, nlohmann::json const& context)
    {
        ...
        if(toolName == "set_track_inputs")
        {
            ...
            for(auto const& trackJson : tracks)
            {
                ...
                {
                    return createError("The 'sender_track_identifier' field is required and must be a string.");
                }
                auto const receiverTrackIdentifier = juce::String(trackJson.at("receiver_track_identifier").get<std::string>());
                auto const receiverInputIdentifier = juce::String(trackJson.at("receiver_input_identifier").get<std::string>());
                auto const senderTrackIdentifier = juce::String(trackJson.at("sender_track_identifier").get<std::string>());
                ...
                else if(!Document::Tools::hasTrackAcsr(documentAcsr, senderTrackIdentifier))
                {
                    response["isError"] = true;
