...
bool Application::Neuralyzer::Mcp::Server::setCopilotApplicationEnabled([[maybe_unused]] bool enabled)
{
    ...
    if(enabled)
    {
        auto const executable = juce::File::getSpecialLocation(juce::File::SpecialLocationType::currentExecutableFile).getFullPathName().toStdString();
        json["servers"]["partiels"] = nlohmann::json{{"type", "stdio"}, {"command", executable}, {"args", nlohmann::json::array({"--mcp-host"})}};
    }
