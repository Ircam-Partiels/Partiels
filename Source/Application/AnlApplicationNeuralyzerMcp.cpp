...
namespace Application::Neuralyzer::Mcp
{
    ...
    static bool waitUntil(double waitSeconds, Predicate&& predicate)
    {
        if(predicate())
        {
            return true;
        ...
        auto const deadline = std::chrono::steady_clock::now() + std::chrono::duration<double>(waitSeconds);
    ...
    static nlohmann::json readResource(nlohmann::json const& request)
    {
        ...
        if(uri == std::string("partiels://available_plugins_list"))
        {
            ...
            auto const [pluginMap, errors] = scanner.getPlugins(48000.0);
    ...
    static std::vector<juce::Range<double>> mergeTimeRanges(std::vector<juce::Range<double>> timeRanges)
    {
        if(timeRanges.empty())
        {
            return timeRanges;
        ...
        std::sort(timeRanges.begin(), timeRanges.end(),
                  [](juce::Range<double> const& lhs, juce::Range<double> const& rhs)
                  {
                      return lhs.getStart() < rhs.getStart();
                  });

        std::vector<juce::Range<double>> merged;
        merged.push_back(timeRanges.front());
        for(auto index = 1_z; index < timeRanges.size(); ++index)
        {
            auto const& range = timeRanges.at(index);
            if(range.getStart() > merged.back().getEnd())
            {
                merged.push_back(range);
            }
            else
            {
                auto const start = merged.back().getStart();
                merged.back() = {start, std::max(merged.back().getEnd(), range.getEnd())};
            }
        }
        return merged;
    }

    static std::variant<Track::Results, juce::String> loadResultsFromCsvString(std::string const& csv, char separator)
    {
        juce::TemporaryFile temp(".csv");
        auto const file = temp.getFile();
        if(!file.replaceWithText(csv, false, false, "\n"))
        {
            return "Failed to create temporary CSV file";
        }

        auto const [fileDescriptionResult, fileDescription] = Track::Loader::getFileDescription(file, 48000.0);
        if(fileDescriptionResult.failed())
        {
            return fileDescriptionResult.getErrorMessage();
        }

        auto csvDescription = fileDescription;
        csvDescription.format = Track::FileDescription::Format::csv;
        csvDescription.columnSeparator = Track::FileDescription::toColumnSeparator(separator);

        std::atomic<bool> shouldAbort{false};
        std::atomic<float> advancement{0.0f};
        return Track::Loader::loadFromFile(csvDescription, shouldAbort, advancement);
    }

    template <typename FrameType>
    static void replaceFramesInRange(std::vector<FrameType>& destination, std::vector<FrameType> const& source, juce::Range<double> const& range)
    {
        auto const destinationStart = std::lower_bound(destination.begin(), destination.end(), range.getStart(), Track::Result::lower_cmp<FrameType>);
        auto const destinationEnd = std::upper_bound(destinationStart, destination.end(), range.getEnd(), Track::Result::upper_cmp<FrameType>);

        auto const sourceStart = std::lower_bound(source.cbegin(), source.cend(), range.getStart(), Track::Result::lower_cmp<FrameType>);
        auto const sourceEnd = std::upper_bound(sourceStart, source.cend(), range.getEnd(), Track::Result::upper_cmp<FrameType>);

        auto const insertionPoint = destination.erase(destinationStart, destinationEnd);
        destination.insert(insertionPoint, sourceStart, sourceEnd);
    }

    template <typename ChannelFrames>
    static juce::Result applyResultsForChannels(std::vector<ChannelFrames>& destination, std::vector<ChannelFrames> const& source, std::set<size_t> const& channels, std::vector<juce::Range<double>> const& mergedTimeRanges)
    {
        auto constexpr invalidChannelMessage = "The selected channels are incompatible with the CSV content.";

        if(channels.empty())
        {
            if(mergedTimeRanges.empty())
            {
                destination = source;
            ...
            {
                return juce::Result::fail(invalidChannelMessage);
            }

            for(auto channelIndex = 0_z; channelIndex < destination.size(); ++channelIndex)
            {
                for(auto const& timeRange : mergedTimeRanges)
                {
                    replaceFramesInRange(destination.at(channelIndex), source.at(channelIndex), timeRange);
                }
            }
            return juce::Result::ok();
        }

        std::vector<size_t> selectedChannels(channels.cbegin(), channels.cend());
        if(source.size() != selectedChannels.size())
        {
            return juce::Result::fail(invalidChannelMessage);
        }

        for(auto csvChannelIndex = 0_z; csvChannelIndex < selectedChannels.size(); ++csvChannelIndex)
        {
            auto const targetChannelIndex = selectedChannels.at(csvChannelIndex);
            if(targetChannelIndex >= destination.size())
            {
                return juce::Result::fail(invalidChannelMessage);
            }
            ...
            if(mergedTimeRanges.empty())
            {
                destination.at(targetChannelIndex) = source.at(csvChannelIndex);
            }
            else
            {
                for(auto const& timeRange : mergedTimeRanges)
                {
                    replaceFramesInRange(destination.at(targetChannelIndex), source.at(csvChannelIndex), timeRange);
                }
            }
        }
    ...
    static juce::Result applyCsvResultsToTrack(Track::Accessor& trackAcsr,
                                               Track::Results const& sourceResults,
                                               std::set<size_t> const& channels,
                                               std::vector<juce::Range<double>> const& mergedTimeRanges)
    {
        auto destinationResults = trackAcsr.getAttr<Track::AttrType::results>();
        auto const access = destinationResults.getWriteAccess();
        if(!static_cast<bool>(access))
        {
            return juce::Result::fail("The result data are being processed");
        }

        if(auto destinationMarkers = destinationResults.getMarkers())
        {
            auto const sourceMarkers = sourceResults.getMarkers();
            if(sourceMarkers == nullptr)
            {
                return juce::Result::fail("The CSV data type does not match the track type.");
            }
            auto const result = applyResultsForChannels(*destinationMarkers, *sourceMarkers, channels, mergedTimeRanges);
            if(result.failed())
            {
                return result;
            }
        }
        else if(auto destinationPoints = destinationResults.getPoints())
        {
            auto const sourcePoints = sourceResults.getPoints();
            if(sourcePoints == nullptr)
            {
                return juce::Result::fail("The CSV data type does not match the track type.");
            }
            auto const result = applyResultsForChannels(*destinationPoints, *sourcePoints, channels, mergedTimeRanges);
            if(result.failed())
            {
                return result;
            }
        }
        else if(auto destinationColumns = destinationResults.getColumns())
        {
            auto const sourceColumns = sourceResults.getColumns();
            if(sourceColumns == nullptr)
            {
                return juce::Result::fail("The CSV data type does not match the track type.");
            }
            auto const result = applyResultsForChannels(*destinationColumns, *sourceColumns, channels, mergedTimeRanges);
            if(result.failed())
            {
                return result;
            }
        ...
        {
            return juce::Result::fail("The track has no editable result data.");
        }

        trackAcsr.setAttr<Track::AttrType::results>(destinationResults, NotificationType::synchronous);
        return juce::Result::ok();
    }
    ...
    static nlohmann::json performToolsCall(nlohmann::json const& request, nlohmann::json const& context)
    {
            ...
            for(auto const& groupIdentifier : documentLayout)
            {
                ...
                for(auto const& trackIdentifier : groupLayout)
                {
                    ...
                    if(deep)
                    {
                        ...
                        {
                            track["input"] = trackAcsr.getAttr<Track::AttrType::input>();
                        }
        ...
        if(toolName == "get_track_results_raw")
        {
            ...
            if(arguments.contains("includeHeader") && !arguments.at("includeHeader").is_boolean())
            {
                return createError("The 'includeHeader' field must be a boolean.");
            }
            auto const includeHeader = arguments.value("includeHeader", false);
            ...
            auto const waitSeconds = arguments.value("waitSeconds", 0.0);
            if(waitSeconds < 0.0)
            {
                return createError("The 'waitSeconds' field must be a non-negative number.");
            }
            ...
            for(auto const& identifierJson : identifiers)
            {
                ...
                waitUntil(waitSeconds,
                          [&]()
                          {
                              frameType = Track::Tools::getFrameType(trackAcsr);
                              return frameType.has_value();
                          });
                if(!frameType.has_value())
                {
                    response["isError"] = true;
        ...
        if(toolName == "set_track_results_raw")
        {
            ...
            if(!arguments.contains("results") || !arguments.at("results").is_array())
            {
                return createError("The 'results' argument is required and must be an array of CSV strings.");
            }

            auto const& identifiers = arguments.at("identifiers");
            auto const& resultsCsv = arguments.at("results");
            if(identifiers.size() != resultsCsv.size())
            {
                return createError("The 'identifiers' and 'results' arrays must have the same size.");
            }
            ...
            auto const mergedTimeRanges = mergeTimeRanges(std::get<0>(timeRangesResults));
            ...
            for(auto index = 0_z; index < identifiers.size(); ++index)
            {
                auto const& identifierJson = identifiers.at(index);
                auto const& csvJson = resultsCsv.at(index);
                if(!identifierJson.is_string())
                {
                    return createError("The 'identifiers' argument is required and must be an array of strings.");
                }
                if(!csvJson.is_string())
                {
                    return createError("The 'results' argument is required and must be an array of CSV strings.");
                }
                ...
                {
                    response["isError"] = true;
                    messages.add(juce::String(R"(The track "TRACKID" doesn't exist.)").replace("TRACKID", identifier));
                    break;
                }

                auto loadedResults = loadResultsFromCsvString(csvJson.get<std::string>(), separator);
                if(auto* error = std::get_if<juce::String>(&loadedResults))
                {
                    response["isError"] = true;
                    messages.add(juce::String(R"(Failed to parse CSV for track "TRACKID": REASON)").replace("TRACKID", identifier).replace("REASON", *error));
                    break;
                }

                auto& trackAcsr = Document::Tools::getTrackAcsr(documentAcsr, identifier);
                auto const result = applyCsvResultsToTrack(trackAcsr, std::get<Track::Results>(loadedResults), channels, mergedTimeRanges);
                if(result.failed())
                {
                    response["isError"] = true;
                    messages.add(juce::String(R"(Failed to update track "TRACKID": REASON)").replace("TRACKID", identifier).replace("REASON", result.getErrorMessage()));
                    break;
                }

                updated.push_back(identifier);
                messages.add(juce::String(R"(Updated raw results for track "TRACKID".)").replace("TRACKID", identifier));
            }

            if(!response.at("isError").get<bool>())
            {
                actionGuard.commit(juce::translate("Set track raw results (Neuralyzer)"));
            }
            else
            {
                actionGuard.abort();
            ...
            content["type"] = "text";
            nlohmann::json payload;
            payload["updated"] = std::move(updated);
            payload["messages"] = messages.joinIntoString("\n");
            content["text"] = payload.dump();
            response["content"].push_back(std::move(content));
            return response;
        }
        if(toolName == "get_track_results_summary")
        {
            ...
            auto effectiveTimeRanges = timeRanges;
            if(effectiveTimeRanges.empty())
            {
                effectiveTimeRanges.emplace_back(-1.0, std::numeric_limits<double>::max());
            }
            std::sort(effectiveTimeRanges.begin(), effectiveTimeRanges.end(),
                      [](juce::Range<double> const& lhs, juce::Range<double> const& rhs)
                      {
                          return lhs.getStart() < rhs.getStart();
                      });
            std::vector<juce::Range<double>> mergedTimeRanges;
            for(auto const& range : effectiveTimeRanges)
            {
                if(mergedTimeRanges.empty() || range.getStart() > mergedTimeRanges.back().getEnd())
                {
                    mergedTimeRanges.push_back(range);
                }
                else
                {
                    auto& back = mergedTimeRanges.back();
                    back = {back.getStart(), std::max(back.getEnd(), range.getEnd())};
                }
            }
        ...
        if(toolName == "set_track_inputs")
        {
            ...
            for(auto const& trackJson : tracks)
            {
                ...
                if(Document::Tools::hasTrackAcsr(documentAcsr, identifier))
                {
                    ...
                    {
                        response["isError"] = true;
                        results.add(juce::String(R"(The input track "TRACKID" doesn't exist.)").replace("TRACKID", input));
                        break;
                    }
                    auto& trackAcsr = Document::Tools::getTrackAcsr(documentAcsr, identifier);
                    if(!Track::Tools::supportsInputTrack(trackAcsr))
                    {
                        response["isError"] = true;
                        results.add(juce::String(R"(The track "TRACKID" doesn't support input.)").replace("TRACKID", identifier));
                        break;
                    }
                    if(!hierarchyManager.isTrackValidFor(identifier, input))
                    {
                        response["isError"] = true;
                        results.add(juce::String(R"(The track "TRACKID" doesn't support input "INPUTID" (unsupported format or circular dependency).)").replace("TRACKID", identifier).replace("INPUTID", input));
                        break;
                    }
                    trackAcsr.setAttr<Track::AttrType::input>(input, NotificationType::synchronous);
                    if(input.isEmpty())
                    {
                        results.add(juce::String(R"(The track "TRACKID" input has been disconnected.)").replace("TRACKID", identifier));
                    }
                    else
                    {
                        results.add(juce::String(R"(The track "TRACKID" now uses "INPUT" as input.)").replace("TRACKID", identifier).replace("INPUT", input));
                    }
                }
                else
                {
        ...
        if(toolName == "create_tracks_with_plugins")
        {
            ...
            if(!layout.empty())
            {
                ...
                for(auto const& trackJson : tracks)
                {
                    ...
                    results.add(juce::String(R"(Created track "TRACKID" with plugin {"identifier":"PLUGINID", "feature":"PLUGINFEATURE"}.)").replace("TRACKID", identifier).replace("PLUGINID", pluginIdentifier).replace("PLUGINFEATURE", pluginFeature));
                }
                if(!response.at("isError").get<bool>())
                {
        ...
        if(toolName == "create_tracks_with_files")
        {
            ...
            if(!layout.empty())
            {
                ...
                for(auto const& trackJson : tracks)
                {
                    ...
                    auto const group = juce::String(trackJson.value("groupIdentifier", layout.front()));
                    auto const groupId = references.count(group) > 0_z ? references.at(group) : group;
...
nlohmann::json Application::Neuralyzer::Mcp::Dispatcher::callMethod(nlohmann::json const& request)
{
...
std::vector<common_chat_tool> Application::Neuralyzer::Mcp::Dispatcher::getToolList()
{
    nlohmann::json request;
...
std::pair<bool, std::vector<std::string>> Application::Neuralyzer::Mcp::Dispatcher::callTools(std::vector<common_chat_tool_call> const& toolCalls)
{
    ...
    for(auto const& toolCall : toolCalls)
    {
        ...
        try
        {
            auto const dispatcherResponse = callMethod(request);
