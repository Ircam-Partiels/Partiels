...
nlohmann::json Application::Neuralyzer::Mcp::createError(const char* what)
{
...
namespace Application::Neuralyzer::Mcp
{
    ...
    static nlohmann::json listResources()
    {
        ...
        for(auto i = 0; i < AnlNeuralyzerTutorialData::namedResourceListSize; ++i)
        {
            ...
            resource["title"] = juce::File::createFileWithoutCheckingPath(AnlNeuralyzerTutorialData::originalFilenames[i]).getFileNameWithoutExtension();
    ...
    static nlohmann::json performToolsCall(nlohmann::json const& methodParams)
    {
        ...
        if(toolName == "get_group_vertical_zoom")
        {
            ...
            auto zoomAcsr = Track::Tools::getVerticalZoomAccessor(trackAcsr.value().get(), false);
            if(!zoomAcsr.has_value())
            {
                return createError("The requested zoom accessor is not available.");
            }
        ...
        if(toolName == "create_tracks_with_plugins")
        {
            ...
            if(!layout.empty())
            {
                ...
                for(auto const& trackJson : tracks)
                {
                    ...
                    auto const numTracks = [&]()
                    {
                        if(Document::Tools::hasGroupAcsr(documentAcsr, groupId))
                        {
                            auto const& groupAcsr = Document::Tools::getGroupAcsr(documentAcsr, groupId);
                            return groupAcsr.getAttr<Group::AttrType::layout>().size();
                        }
                        return 0_z;
                    }();
                    auto const trackPosition = numTracks - trackJson.value("position", numTracks);
                    auto const [result, identifier] = Tools::addPluginTrack(groupId, trackPosition, pluginKey);
                    if(result.failed())
                    {
                        response["isError"] = true;
        ...
        if(toolName == "create_tracks_with_files")
        {
            ...
            if(!layout.empty())
            {
                ...
                for(auto const& trackJson : tracks)
                {
                    ...
                    auto const trackPosition = numTracks - trackJson.value("position", numTracks);
                    auto const [result, identifier] = Tools::addFileTrack(groupId, trackPosition, file);
                    if(result.failed())
                    {
                        response["isError"] = true;
...
nlohmann::json Application::Neuralyzer::Mcp::Dispatcher::callMethod(nlohmann::json const& request, DynamicToolMethods const& methods)
{
    ...
    if(method == "tools/list")
    {
        ...
        auto& tools = list["tools"];
        // Remove unsupported tools
        auto it = std::remove_if(tools.begin(), tools.end(), [&](auto const& tool)
                                 {
                                     if(!tool.contains("name") || !tool.at("name").is_string())
                                     {
                                         return true;
                                     }
                                     auto const& name = tool.at("name").template get<std::string>();
                                     if(name == "read_files")
                                     {
    ...
    if(method == "tools/call")
    {
            ...
            auto const results = juce::MessageManager::callSync([&]() -> std::tuple<std::map<Plugin::Key, Plugin::Description>, juce::StringArray>
                                                                {
                                                                    ...
                                                                    {
                                                                        return std::make_tuple(std::map<Plugin::Key, Plugin::Description>{}, juce::StringArray{"Unkown error"});
                                                                    }
                                                                });
            nlohmann::json response;
            response["isError"] = false;
        ...
        if(toolName == "get_vamp_plugin_descriptions")
        {
            ...
            auto const keys = arguments.at("keys").get<std::vector<Plugin::Key>>();
            auto const results = juce::MessageManager::callSync([&]() -> std::tuple<std::map<Plugin::Key, Plugin::Description>, juce::StringArray>
                                                                {
                                                                    try
                                                                    {
...
std::vector<std::pair<std::string, std::string>> Application::Neuralyzer::Mcp::Dispatcher::getResources()
{
    ...
    for(auto const& resource : listResults.at("resources"))
    {
        ...
        if(resource.contains("title") && resource.at("title").is_string() && resource.contains("uri") && resource.at("uri").is_string())
        {
            ...
            {
                auto const title = resource.at("title").get<std::string>();
                for(auto const& content : contentResult.at("contents"))
                {
                    ...
                    {
                        resources.push_back(std::make_pair(title, content.at("text").get<std::string>()));
                    }
                }
            }
        }
...
std::vector<common_chat_tool> Application::Neuralyzer::Mcp::Dispatcher::getToolList(DynamicToolMethods const& methods)
{
    nlohmann::json request;
    request["method"] = "tools/list";
    auto const response = callMethod(request, methods);
...
common_chat_msg Application::Neuralyzer::Mcp::Dispatcher::callTool(DynamicToolMethods const& methods, common_chat_tool_call const& toolCall)
{
    nlohmann::json request;
    ...
    try
    {
        message.content = callMethod(request, methods).dump();
    }
    catch(std::exception const& e)
    {
        message.content = createError(std::string("Exception during execution: ") + e.what());
    }
    catch(...)
    {
        message.content = createError(std::string("Exception during execution: Unknown error"));
    }
    return message;
}
...
, mDispatcher(dispatcher)
{
    mListener.onAttrChanged = []([[maybe_unused]] Accessor const& acsr, AttrType attr)
    {
        switch(attr)
        {
            ...
            {
                setClaudeApplicationEnabled(acsr.getAttr<AttrType::mcpForClaudeApp>());
                break;
            }
            case AttrType::mcpForCopilotApp:
            {
                setCopilotApplicationEnabled(acsr.getAttr<AttrType::mcpForCopilotApp>());
                break;
            }
        }
    };
...
bool Application::Neuralyzer::Mcp::Host::run(int port)
{
    ...
    public:
        ...
        void sendMessage(nlohmann::json const& message)
        {
            ...
            MiscDebug("Application::Neuralyzer::Mcp::Host", "MCP host send message: " + text);
    ...
    if(!clientConnection->connectToSocket("127.0.0.1", port, 10000))
    {
