...
namespace Application::Neuralyzer::Mcp
{
    ...
    static nlohmann::json performToolsCall(nlohmann::json const& request, nlohmann::json const& context)
    {
        ...
        if(toolName == "get_track_vertical_zoom")
        {
            ...
            {
                return createError(juce::String("The track \"TRACKID\" doesn't exist.").replace("TRACKID", identifier).toStdString());
            }
        ...
        if(toolName == "get_group_vertical_zoom")
        {
            ...
            {
                return createError(juce::String("The group \"GROUPID\" doesn't exist.").replace("GROUPID", identifier).toStdString());
            }
        ...
        if(toolName == "get_track_results_raw")
        {
            ...
            if(arguments.contains("timeRange"))
            {
                if(!arguments.at("timeRange").is_array())
                {
                    return createError("The 'timeRange' field must be an array of two numbers [startTime, endTime].");
                }
                auto const& timeRangeArray = arguments.at("timeRange");
                if(timeRangeArray.size() != 2)
                {
                    return createError("The 'timeRange' field must have exactly 2 elements [startTime, endTime].");
                }
                if(!timeRangeArray[0].is_number() || !timeRangeArray[1].is_number())
                {
                    return createError("The 'timeRange' elements must be numbers.");
                }
                auto const startTime = timeRangeArray[0].get<double>();
                auto const endTime = timeRangeArray[1].get<double>();
                if(startTime > endTime)
                {
                    return createError("The 'timeRange' startTime must be less than or equal to endTime.");
                }
                timeRange = juce::Range<double>(startTime, endTime);
            }
            ...
            auto effectiveTimeRange = timeRange;
            if(effectiveTimeRange.isEmpty())
            {
                effectiveTimeRange = {-1.0, std::numeric_limits<double>::max()};
            }
            ...
            for(auto const& identifierJson : identifiers)
            {
                ...
                if(!Document::Tools::hasTrackAcsr(documentAcsr, identifier))
                {
                    ...
                    content["text"] = juce::String("The track \"TRACKID\" doesn't exist.").replace("TRACKID", identifier);
                ...
                else
                {
                    ...
                    if(!static_cast<bool>(access))
                    {
                        ...
                        content["type"] = "text";
                        content["trackId"] = identifier;
                        content["text"] = juce::translate("The result data are being processed").toStdString();
                        response["content"].push_back(std::move(content));
                        continue;
                    }
                    ...
                    if(!allowLargeOutput)
                    {
                        if(results.isEmpty())
                        {
                            ...
                            content["type"] = "text";
                            content["trackId"] = identifier;
                            content["text"] = juce::translate("The result data are invalid").toStdString();
                            response["content"].push_back(std::move(content));
                            continue;
                        }
                        ...
                        auto const countResultsInChannels = [&](auto const& allResults)
                        {
                            ...
                            for(auto channelIndex = 0_z; channelIndex < allResults->size(); ++channelIndex)
                            {
                                ...
                                auto it = std::lower_bound(channelResults.cbegin(),
                                                           channelResults.cend(),
                                                           effectiveTimeRange.getStart(),
                                                           Track::Result::lower_cmp<result_type>);
                                while(it != channelResults.cend() && std::get<0_z>(*it) <= effectiveTimeRange.getEnd())
                                {
                    ...
                    auto const result = Track::Exporter::toCsv(trackAcsr,
                                                               timeRange,
                                                               channels,
                                                               stream,
                                                               includeHeader,
                    ...
                    {
                        response["isError"] = true;
                        content["text"] = result.getErrorMessage().toStdString();
                    }
                    else
                    {
        ...
        if(toolName == "get_track_results_summary")
        {
            ...
            for(auto const& identifierJson : identifiers)
            {
                ...
                {
                    response["isError"] = true;
                    trackStats["error"] = juce::String("The track \"TRACKID\" doesn't exist.").replace("TRACKID", identifier);
                ...
                {
                    response["isError"] = true;
                    trackStats["error"] = juce::translate("The result data are being processed").toStdString();
                    stats.push_back(std::move(trackStats));
                    continue;
                }
                if(results.isEmpty())
                {
                    response["isError"] = true;
                    trackStats["error"] = juce::translate("The result data are invalid").toStdString();
                    stats.push_back(std::move(trackStats));
                    continue;
                }
                ...
                auto const iterateResults = [&](std::string const& name, auto const& allResults, std::function<void(void)> onChannelBegin, std::function<void(typename std::decay_t<decltype(allResults)>::element_type::value_type::value_type const&)> onResultAdvance, std::function<nlohmann::json(void)> onChannelEnd)
                {
                    ...
                    for(auto channelIndex = 0_z; channelIndex < allResults->size(); ++channelIndex)
                    {
                        ...
                        if(channels.empty() || channels.count(channelIndex) > 0_z)
                        {
                            ...
                            channelPayload["value"] = nlohmann::json::object();
                            channelPayload["extraValues"] = nlohmann::json::array();
                            if(onChannelBegin != nullptr)
                            {
                                onChannelBegin();
                            }
                            ...
                            auto const& channelResults = allResults->at(channelIndex);
                            auto it = std::lower_bound(channelResults.cbegin(), channelResults.cend(), effectiveTimeRange.getStart(), Track::Result::lower_cmp<result_type>);
                            // Ierate over results
                            while(it != channelResults.cend() && std::get<0_z>(*it) <= effectiveTimeRange.getEnd())
                            {
                            ...
                            channelPayload["count"] = resultCount;
                            for(auto index = 0_z; index < extraValues.size(); ++index)
                            {
                                ...
                                else
                                {
                                    ...
                                    extraValueStats["name"] = juce::translate("Extra INDEX").replace("INDEX", juce::String(index + 1_z));
                                    extraValueStats["unit"] = "";
                                }
                                extraValueStats["stats"] = statisticsPayload(extraValues.at(index));
                                channelPayload["extraValues"].push_back(std::move(extraValueStats));
                            }

                            if(onChannelEnd != nullptr)
                            {
                                channelPayload["value"] = onChannelEnd();
                            }
        ...
        if(toolName == "create_tracks_with_files")
        {
            ...
            if(!layout.empty())
            {
                ...
                for(auto const& trackJson : tracks)
                {
                    ...
                    if(!file.existsAsFile() || !file.hasReadAccess())
                    {
                        return createError(juce::String("The file 'FILEPATH' does not exist or is not readable.").replace("FILEPATH", file.getFullPathName()).toStdString());
                    }
                    ...
                    {
                        response["isError"] = true;
                        results.add(juce::String("Failed to create track in group \"ID\" from file \"FILEPATH\": REASON Ensure the file path is valid and readable, and that the group exists. Consider calling the tool 'get_document_state' to get the correct group identifiers.").replace("ID", groupId).replace("FILEPATH", file.getFullPathName()).replace("REASON", result.getErrorMessage()));
                        break;
                    }
                    trackIdentifiers.insert(identifier);
                    created.push_back(identifier);
                    results.add(juce::String("Created track \"TRACKID\" from file \"FILEPATH\".").replace("TRACKID", identifier).replace("FILEPATH", file.getFullPathName()));
                }
                if(!response.at("isError").get<bool>())
                {
                    Tools::revealTracks(trackIdentifiers);
    ...
    for(auto const& toolCall : toolCalls)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                auto const text = content[0].value("text", "(empty)");
