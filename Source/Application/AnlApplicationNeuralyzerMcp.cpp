...
#include <AnlNeuralyzerRagData.h>
...
{
    static auto constexpr partielsManualResourceUri = "partiels://manual";
    static auto constexpr partielsTutorial1ResourceUri = "partiels://howtodisplayandconfigureaFouriertransformspectrogram";
    ...
    static nlohmann::json listResources()
    {
        ...
        result["resources"] = nlohmann::json::array();
        {
            ...
            resource["uri"] = partielsManualResourceUri;
            resource["name"] = "Partiels Manual";
            resource["title"] = "Partiels User Manual";
            resource["description"] = "User manual for Partiels";
            resource["mimeType"] = "text/markdown";
            result["resources"].push_back(std::move(resource));
        }
        {
            nlohmann::json resource;
            resource["uri"] = partielsTutorial1ResourceUri;
            resource["name"] = "Partiels Tutorials - 1";
            resource["title"] = "How to display and configure a Fourier transform spectrogram?";
            resource["description"] = "How to display and configure a Fourier transform spectrogram using Partiels?";
            resource["mimeType"] = "text/markdown";
            result["resources"].push_back(std::move(resource));
        }
    ...
    static nlohmann::json readResource(nlohmann::json const& request)
    {
        ...
        if(uri == partielsManualResourceUri)
        {
            nlohmann::json result;
            result["contents"] = nlohmann::json::array();

            nlohmann::json content;
            content["uri"] = partielsManualResourceUri;
            content["mimeType"] = "text/markdown";
            content["text"] = juce::String::fromUTF8(AnlNeuralyzerRagData::PartielsManual_md, AnlNeuralyzerRagData::PartielsManual_mdSize);
            result["contents"].push_back(std::move(content));

            return result;
        }
        if(uri == partielsTutorial1ResourceUri)
        {
            nlohmann::json result;
            result["contents"] = nlohmann::json::array();

            nlohmann::json content;
            content["uri"] = partielsTutorial1ResourceUri;
            content["mimeType"] = "text/markdown";
            content["text"] = juce::String::fromUTF8(AnlNeuralyzerRagData::HowtodisplayandconfigureaFouriertransformspectrogram_md, AnlNeuralyzerRagData::HowtodisplayandconfigureaFouriertransformspectrogram_mdSize);
            result["contents"].push_back(std::move(content));
    ...
    static nlohmann::json performToolsCall(nlohmann::json const& request, nlohmann::json const& context)
    {
                    ...
                    else
                    {
                        if(auto const markers = trackResults.getMarkers())
                        {
                            ...
                            for(auto const& channelMarkers : *markers)
                            {
                                ...
                                {
                                    auto const markerTime = std::get<0_z>(marker);
                                    auto const markerDuration = std::get<1_z>(marker);
                                    if(!timeRange.isEmpty() && markerTime + markerDuration < timeRange.getStart())
                                    {
                                        continue;
                                    }
                                    if(!timeRange.isEmpty() && markerTime > timeRange.getEnd())
                                    {
                                        continue;
                                    }
                                    nlohmann::json frame;
                                    frame["time"] = markerTime;
                                    frame["duration"] = markerDuration;
                        ...
                        else if(auto const points = trackResults.getPoints())
                        {
                            ...
                            for(auto const& channelPoints : *points)
                            {
                                ...
                                {
                                    auto const pointTime = std::get<0_z>(point);
                                    auto const pointDuration = std::get<1_z>(point);
                                    if(!timeRange.isEmpty() && pointTime + pointDuration < timeRange.getStart())
                                    {
                                        continue;
                                    }
                                    if(!timeRange.isEmpty() && pointTime > timeRange.getEnd())
                                    {
                                        continue;
                                    }
                                    nlohmann::json frame;
                                    frame["time"] = pointTime;
                                    frame["duration"] = pointDuration;
        ...
        if(toolName == "create_tracks_with_files")
        {
            ...
            if(!layout.empty())
            {
                ...
                {
                    Tools::revealTracks(trackIdentifiers);
                    documentDir.endAction(ActionState::newTransaction, juce::translate("Create tracks from files (Neuralyzer)"));
                }
                else
                {
                    documentDir.endAction(ActionState::abort);
