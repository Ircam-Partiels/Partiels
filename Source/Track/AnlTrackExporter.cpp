...
juce::Result Track::Exporter::fromJson(Accessor& accessor, juce::File const& file)
{
    ...
    {
        return juce::Result::fail(juce::translate("The track ANLNAME cannot import the JSON file because the input stream of the file FLNAME cannot be opened.").replace("ANLNAME", name).replace("FLNAME", file.getFullPathName()));
    }
    stream >> json;
    stream.close();
