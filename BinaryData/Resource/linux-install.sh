...
echo "HasRootAccess: " $HasRootAccess
...
if [ "$UseHome" = "1" ]; then
    ...
    if [ "$HasRootAccess" = "0" ]; then
        echo '\033[0;31m' "Running as a regular user, if you wish to do a user-local install, please run this installer as a root (or install sudo)."
        echo '\033[0m'
        exit -1
    fi
