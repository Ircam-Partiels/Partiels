...
## Analyze the pitch of the audio
...
## Analyze the fist N chord partials
...
3. Get the raw results from the marker track (allow large output if necessary)
4. Get a summary of the pitch track results for the time interval corresponding to each marker, or between two consecutive markers if the duration is zero, and use the statistics (in hertz) to estimate the pitch

### Tips: 
* If you created the pitch track, get the summary of results (wait a few seconds if necessary) and adjust the confidence threshold to exclude results with a low score
* If you created the marker track, get the summary of results (wait a few seconds if necessary) and adjust the energy threshold to exclude results with a low energy